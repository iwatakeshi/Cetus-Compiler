...
class Typecheck : public Visitor{
  private:
    ...
    void add_proc_symbol(ProcImpl* p){
        ...
        if (strcmp(name, "Main") == 0){
            // check if  there is duplicated Main in the source program
            if (main_inserted)
                this -> t_error(no_main, p -> m_attribute);
            else{
                m_st -> insert(name, s);
    ...
    // Check that if the right-hand side is an lhs, such as in case of addressof
  ...
  public:
    ...
    void visitProcImpl(ProcImpl* p){
        ...
        default_rule(p);
        m_st -> close_scope();
        // checks for a ret_type_mismatch
        check_proc(p);
    }
